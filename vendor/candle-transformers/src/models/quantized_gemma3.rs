...
// MAX_SEQ_LEN only sizes the pre-allocated rotary-embedding tables (sin/cos), NOT the model's
// supported context. Upstream is 128K, which pre-allocates ~4.3 GB of rotary tables across the 34
// layers (head_dim=256) — every layer holds its own RotaryEmbedding with full-window sin+cos. On a
// 6 GB card (RTX 4050) that alone blows VRAM before the weights even load, forcing PCIe paging and
// collapsing hashrate to ~0.4 MH/s. keryx mining inference is hard-capped at 2048 tokens (slm.rs
// generate), so 8192 is a 4× headroom cap that cuts the rotary allocation ~16× (~4.3 GB → ~270 MB)
// while keeping any realistic mining sequence in-bounds.
...
impl ModelWeights {
    ...
    /// PoM zero-dup support (keryx fork): the quantized weight matrices held resident in VRAM,
    /// keyed by their canonical GGUF name. The QMatMul-backed tensors keep candle's raw
    /// quantized bytes — exactly what the node's R_T commits — so the PoM possession walk can
    /// gather from these same resident buffers instead of loading a duplicate ~2GB copy. Only
    /// the QMatMul weights are returned; the Embedding/RmsNorm tensors are dequantized at load
    /// (not raw quantized), so the PoM loader reads those small ones fresh.
    ///
    /// Mirrors `quantized_llama_split::pom_quant_tensors`. Additive accessor: it touches neither
    /// loading nor the forward path. Gemma3 GGUF tensor names match Llama's (`blk.N.attn_q.weight`
    /// etc.), so the PoM loader's name-keyed reuse works unchanged.
    pub fn pom_quant_tensors(&self) -> std::collections::HashMap<String, std::sync::Arc<QTensor>> {
        fn inner(qmm: &QMatMul) -> Option<std::sync::Arc<QTensor>> {
