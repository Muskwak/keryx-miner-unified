...
impl QTensor {
    ...
    /// Zero-dup accessor for the Metal-resident quantized storage.
    /// Vendored patch: upstream 0.9.2 keeps `storage` private and `device_ptr`
    /// bails on the Metal branch, forcing consumers to blit weights out of the
    /// unified-memory MTLBuffer. Returning `&QMetalStorage` (which already has a
    /// public `buffer()` accessor) lets the PoM Metal miner build a bindless
    /// pointer table over candle's own buffers with no packing / copying.
    #[cfg(feature = "metal")]
