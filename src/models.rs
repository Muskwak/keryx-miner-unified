...
// the pinned `(root, N)` for the model it mines (see `pom_gpu_vulkan_desktop::ensure_installed`),
// so a wrong-quant / corrupt / truncated GGUF is caught once at index-build time — instead of
// silently producing PoM blocks every one of which the node rejects with `BadWeightPath`. Keyed by
// model_id (not slice position) so the check stays correct even if the node later reorders tiers.
//
// The CUDA/Metal backends don't currently use this guard (they trust the GGUF + the host index's
// own N self-check); the Vulkan desktop backend, ported from keryx-miner-rdna3, added it because
// llama.cpp's resident weight layout is engine-internal and a mismatch there is otherwise silent.
//
// TODO(Phase 4): the per-model `(R_T, N)` anchors below MUST be copied VERBATIM from the node's
// `POM_TIERS` / `POM_TIERS_H2` tables (keryx-node consensus/core/src/config/params.rs) before the
// desktop Vulkan backend mines on mainnet. An empty table is safe (the guard is skipped with a
// warning) but provides no protection — populate it as part of the inference-engine unification
// where the llama.cpp+Vulkan engine lands properly.
...
/// Per-model `(R_T, N)` anchors. Empty until Phase 4 populates them from the node's consensus
/// tables — see the module comment above.
pub const POM_ANCHORS: &[PomAnchor] = &[];

/// The consensus-pinned `(R_T, N)` anchor for `model_id`, if any. `None` (the guard is skipped)
/// when the table is empty or the model isn't pinned.
