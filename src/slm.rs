...
// zero-dup PoM walk reads the engine's own resident weight buffers directly, so the walk module
// needs a handle to the resident engine serving the mining model. On the candle backends (CUDA /
// Metal) inference lives in this module's `ENGINE` (a candle model), not an `LlamaEngine`, so
// there is no llama.cpp engine to hand back — `active_engine` returns `None` there and the Vulkan
// walk falls back to its streamed weight blob (correct, just one extra VRAM copy).
//
// Phase 4 unifies the inference engines behind a real trait so a single `active_engine` serves
// both; until then this seam exists only so the `vulkan`-feature compile of the desktop backend
// resolves. The engine itself (`LlamaEngine::launch` / `chat` / `build_shared_walk`) is in
// `llm_engine.rs`, also gated to `vulkan`.
#[cfg(feature = "vulkan")]
pub fn active_engine(_model_id: &[u8; 32]) -> Option<std::sync::Arc<crate::llm_engine::LlamaEngine>> {
    // No llama.cpp engine is resident on the candle path. The desktop Vulkan walk's `install_shared`
    // treats `None` as "fall back to the streamed blob" — correct behaviour, just without the
    // zero-dup optimization. Phase 4 wires a real resident-engine registry here.
    None
}
